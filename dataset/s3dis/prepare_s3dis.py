...
def preprocess_s3dis (data_dir):
    ...
    for scene_name in scene_list:
        area = scene_name.split ('.') [0]
        name = scene_name.split ('.') [1]
        save_dir = f'dataset/s3dis/preprocess'
        ...
        torch.save((scene_data[:, :3].astype(np.float32), scene_data[:, 3:6].astype(np.float32), semantics.reshape(-1).astype(np.int32), instances.reshape(-1).astype(np.int32)), scene_pth)
